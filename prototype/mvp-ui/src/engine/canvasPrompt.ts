...
**阶段 1 可行性字段**：
- needsPersistence：是否需要保存数据/历史记录
- needsAuth：是否需要登录或多用户协作
- needsIntegration：是否需要对接微信/钉钉/支付等外部系统
- 若任一为 true：用大白话告知「首版将以 mock 演示，真实能力后续版本支持」，设 feasibilityAcknowledged 为 false
- gateHints.requirementsReady 仅当：goal/users/p0/acceptance/timeline 齐全，且（三字段均为 false 或用户已在右侧确认 mock 方案）
